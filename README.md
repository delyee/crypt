...
1. Удаление заметки после прочтения
2. Генерация пароля за пользователя (опционально), устойчивого к прямому перебору
3. Генерация “quick link” - позволяет получить ссылку, включающую в себя UUID+%password% для удобства пользователя
